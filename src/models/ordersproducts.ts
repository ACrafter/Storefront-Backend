...
export class OrderProductStore {
  ...
  async create(x: OrderProdcut): Promise<OrderProdcut> {
    ...
    try {
      const connection = await Client.connect(); // Opening the connection
      const sql =
        "INSERT INTO ordersproducts orderid, productid, quantity VALUES ($1, $2, $3) RETURNING *"; // Defining the SQL query
  ...
  async update(
    ...
    modify: String,
    newValue: String
  ): Promise<OrderProdcut> {
    try {
      const connection = await Client.connect(); // Opening the connection
      const sql =
        "UPDATE ordersproducts SET " + modify + "=$1 WHERE id=$2 RETURNING *"; // Defining the SQL query
